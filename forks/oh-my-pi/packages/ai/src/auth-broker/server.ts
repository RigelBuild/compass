...
import { PROMETHEUS_CONTENT_TYPE, renderUsageMetrics } from "./prometheus-metrics";
...
export interface AuthBrokerServerOptions {
	...
	bearerTokens: string[];
	/**
	 * Scrape-scoped read-only tokens accepted ONLY for `GET /metrics` (SEA-1242).
			...
			try {
				...
				if (req.method === "GET" && pathname === "/metrics") {
					...
					try {
						...
						// capped by the TTL (SEA-1242 §Provider-load model). `?? []` on
						// last-good expiry / absent method renders an empty 200 exposition
						// so the dashboard sees absent series, not a 5xx.
						const reports = (await opts.storage.fetchUsageReports?.({ signal: req.signal })) ?? [];
						const body = renderUsageMetrics(reports);
						...
						// expected-accounts panel detects (SEA-1242 §The /metrics endpoint).
