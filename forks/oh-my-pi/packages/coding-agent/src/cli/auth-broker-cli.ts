...
export interface AuthBrokerCommandArgs {
	...
	flags: {
		...
		bind?: string;
		regenerate?: boolean;
		/** `token`/`serve`: operate on the scrape-scoped `/metrics` token (SEA-1242 OQ6). */
		metrics?: boolean;
		via?: string;
		provider?: string;
		dryRun?: boolean;
...
/**
 * Scrape-scoped read-only token file (SEA-1242 OQ6). Authorizes ONLY
 * `GET /metrics`, never the vault routes, so the fleet's Alloy scrape credential
 * is least-privilege and distinct from the master bearer. A fixed
 * StateDirectory path mirroring the master bearer's convention: this file — not
 * a one-shot stdout print — is the single deterministic source T2's
 * `systemd-creds encrypt` reads at stage time. Rotation: re-mint (rewrites the
 * file), re-encrypt from the same path, re-converge.
 */
...
async function writeTokenFile(file: string, token: string): Promise<void> {
	...
	// No trailing newline: the raw file bytes ARE the bearer value that T2's
	// `systemd-creds encrypt` stages verbatim (SEA-1242 §Token handoff).
...
async function runServe(flags: AuthBrokerCommandArgs["flags"]): Promise<void> {
	...
	// Mint the scrape-scoped read-only token too (SEA-1242 OQ6). It authorizes
	// ONLY `GET /metrics`; the fleet Alloy scrape stages this value, never the
	// master bearer above.
...
async function runToken(flags: AuthBrokerCommandArgs["flags"]): Promise<void> {
	// `--metrics` selects the scrape-scoped read-only token (SEA-1242 OQ6);
	// otherwise this manages the master bearer, unchanged.
