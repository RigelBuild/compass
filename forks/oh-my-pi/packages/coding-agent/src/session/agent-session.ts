...
export class AgentSession {
	 ...
	 * never rejects the next. `#activeRefresh` is the *raw* in-flight
	 * `#doRefresh` run that `requestRestart()` drains — held separately from the
	 * swallowing tail so a failed `onBeforeRefresh` pull surfaces to the restart,
	 * not only to the refresh caller. It is cleared on settlement under an
	 * identity check so it never names a stale (settled/rejected) run.
	 */
	...
	#wakeForIrc(records: CustomMessage[]): void {
		...
		// Callers drained the pending IRC queues into `records` before calling
		// here, so re-queue them rather than drop — they flush to the transcript
		// on a successful restart's dispose (#flushPendingIrcAsides) or stay
		// pending for the resumed session on a recoverable pre-dispose failure.
		if (this.#restarting || this.#isDisposed) {
			this.#pendingIrcAsides.push(...records);
			return;
		}
	...
	async refresh(scope: RefreshScope = "all"): Promise<RefreshResult> {
		// Restart in progress: refuse a new refresh so nothing chains on behind
		// the one restart drains, and report the refusal (not a silent no-op).
		if (this.#restarting) return { refused: "restarting" };
		// Serialize onto the tail so overlapping callers run strictly in order,
		// each seeing a fully-applied prior refresh. Re-check the latch when this
		// callback begins: a refresh queued behind an in-flight one *before* the
		// latch must refuse, not run during restart (decision (k)).
		const run = this.#refreshTail.then((): RefreshResult | Promise<RefreshResult> => {
			if (this.#restarting) return { refused: "restarting" };
			// #activeRefresh is the raw #doRefresh run requestRestart() drains so
			// a failed onBeforeRefresh pull surfaces to the restart, not just to
			// this caller (decision (j)) — never the failure-swallowing tail below.
			const active = this.#doRefresh(scope);
			this.#activeRefresh = active;
			// Clear on settle, identity-checked, so #activeRefresh only ever names
			// a CURRENTLY-running refresh — never a stale settled/rejected promise
			// the next requestRestart() would wrongly drain (decision (j)). Use
			// then(clear, clear), NOT finally: .finally() returns a derived promise
			// that re-raises a rejection, and voiding it leaks an unhandled rejection
			// on a throwing onBeforeRefresh pull — which OMP's process-level handler
			// escalates to a session-wide teardown (#4499/#4672). then(clear, clear)
			// absorbs the rejection on this identity-clear branch; the real failure
			// still propagates through `active`/`run` to the caller and the drain.
			const clearActive = () => {
	...
	async #doRequestRestart(
		...
		try {
			...
			// whole entry->dispose window. The check at 7977 only closes the
