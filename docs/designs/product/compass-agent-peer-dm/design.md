...
Ledger-impact: deferred to freeze. Do not edit DECISIONS.md in this PR; the
caller applies these rows at freeze:

- **DL-new-1 (name addressing):** Agents and client UIs address channels and
...
- **DL-new-2 (reply routing):** Steer/deliver control ops denormalize the
...
- **DL-new-3 (topic gate):** Creating a topic requires `create_topic: true`
...
- **DL-new-4 (DM path):** Agent↔agent DMs are auto-created
...
- **DL-new-5 (DM namespace defense):** The reserved DM group is
...
- **DL-new-6 (DM shape):** A peer-DM is born `kind=DM`, exactly two agent
...
- **DL-new-7 (scope):** Peer-DM scope is same-owner for MVP (the F2
...
DL-new-7). `expandOwnerMembership` would already pull both owners in, so the
