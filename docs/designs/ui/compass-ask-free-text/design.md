...
**Free-text questions are unanswerable.** An `AskQuestion` whose `options` list
is empty is a free-text question — the wire says so: "Free-text answer
(\"Other\"); a question with no options is answered by custom_text alone"
(`AskQuestionAnswer.custom_text` in `proto/compass/v1/comms.proto`). The UI
renders such a question as an empty `.ask-options` container with no input
(`AskBlock` in `apps/ui/src/components/ChannelView.tsx` — the only per-question
content is `<For each={q().options}>`), so a free-text question cannot be
answered. This record designs the free-text answer path end to end: domain
shape, store surface, outbound wiring, renderer, and completion accounting.
...
confirmation — the tail of `answerAsk` in `apps/ui/src/store.ts`; a
single-question ask sends on its only click. Matt's ruling on this record: **an
ask is never auto-sent — submitting a response is always an explicit user
gesture.** That changes **already-shipped behaviour**, not just the new
free-text path: the completing-click send and the single-click single-question
send are removed, and the submit control becomes the sole, unconditional send
path (D0). The existing gate comment argued only against a *per-click* respond
("don't send early"); it never argued "send without confirmation" — auto-send
was simply where the gate landed once partial sends were ruled out. Meanwhile
the wire is one-shot and unrepeatable (`applyAskAnswer`'s answer-once guard in
...
can already complete an ask before the user's second pick, and free text widens
that sharp edge from a rare shape to a common one (typed text counting toward
completeness). Every other destructive one-shot in this UI has an explicit
control; the ask now does too.
...
- **`answerAsk`'s send tail is removed.** Today it ends `if (!answered ||
  !isAskComplete(answered)) return; sendAsk(messageId, answered, before);`. Both
  lines go; `answerAsk` becomes record-only, exactly like the new
  `answerAskText` (D2). The `before` capture (the rollback target) goes with it;
  the `answered` local survives only to drive `clearAskError`.
- **The gate comment is rewritten.** The gate comment inside `answerAsk`
  currently reads "THE GATE (Matt's ruling): the click stays LOCAL until the ask
  is COMPLETE. … A single-question ask completes on its only click, so it still
  sends there." That ruling is superseded: clicks stay local, full stop. The
...
- **`submitAsk` is the sole send path.** Its body barely moves: the
  `isAskSubmitted` / `ask.answered` gates in `submitAsk` stay, and the
  wholly-unanswered guard `if (ask.questions.every((q) =>
  q.chosenOptionIds.length === 0)) return;` becomes `if
  (!ask.questions.some(isQuestionAnswered)) return;` (D5) — a wholly blank
  respond says nothing, so it stays inert. The doc comment above `submitAsk`
  ("The skip affordance. …") is rewritten — it is no longer the affordance for
  the skip case but the send path for every case:
...
- **`isAskComplete` is deleted.** Its only caller was the removed send tail in
  `answerAsk`. Completeness survives solely as UI copy: `AskBlock` computes
  `answeredCount() < ask().questions.length` inline (from the shared
  `isQuestionAnswered`, D5) to switch the submit label — no store predicate
  needed.
- **The `AppStore` interface doc comments follow.** The doc comment on
  `answerAsk` in `apps/ui/src/store.ts` promises "the completing click … a
  single-question ask completes on its only click" and "a REFUSED respond rolls
  the local answer back" — both now false; it shrinks to the record-only
  contract (local recording, first-responder-wins, the no-op gates). The doc
  comment on `submitAsk` ("Submit an INCOMPLETE ask — the skip affordance")
  becomes the sole-send-path contract, matching its body comment above.
- **`ChannelView.tsx`'s component account is rewritten.** The doc comment above
  `AskBlock` in `apps/ui/src/components/ChannelView.tsx` says "clicks accumulate
  locally and the completing click ships them all at once. A question the user
  means to SKIP … grows a `submit` control" — now wrong twice over. Replacement:
...
guarded by `sameAnswers(current, ask)` — both inside `sendAsk` in
`apps/ui/src/store.ts`. Both callers of that machinery die: `answerAsk` no
longer sends, and `submitAsk` never passed a rollback ("nothing was recorded by
this call"). With submit recording nothing, a refusal has nothing to roll *back*
— the staged local state is already honest and retryable. What a refusal can now
cost is the **staged work itself**, via the in-flight-push window (the decided
contract below). So:

- `sendAsk`'s signature simplifies to `sendAsk(messageId, ask)` — the
  `rollback` parameter is deleted.
- `sameAnswers` in `apps/ui/src/store.ts` is deleted: its one caller was the
  rollback guard inside `sendAsk`. The prior draft widened it with a
...
restaging.** The window: submit issues the respond and marks the ask submitted;
`preserveLocalAsks` deliberately skips submitted asks (its `isAskSubmitted`
guard), so a stream push restating the ask blank replaces the local copy —
clicks and typed text both; then the respond is refused and `unmarkAskSubmitted`
reopens the ask over that blank state. Today the same window loses clicks; with
free text it loses typed prose, which is costlier. The shipped `ask` object in
`sendAsk`'s closure **still holds every answer** (it is the exact object
`respondToAsk` serialized), so the catch restages it:
...
marked submitted, both recorders are gated off (`isAskSubmitted` at entry to
`answerAsk` and to `submitAsk` in `apps/ui/src/store.ts`), so no local edit can
land between submit and refusal — the only way `current` differs from `ask` is a
stream push. An *unanswered* push is always blank (the proto: answer values on
an inbound Ask are ignored; the server records chosen ids and `custom_text` and
flips `answered` in one function — `applyAskAnswer` sets `q.CustomText` then
`ask.Answered = true`, persisted by the single `updateMessageBlocksExec` in the
same tx), so restoring the shipped answers over it is exactly what
`preserveLocalAsks` would have done had the ask not been submitted. An
*answered* push is authoritative and the `!current.answered` clause declines,
preserving the accepted-then-lost-reply behaviour the race suite pins. The
`current !== ask` reference check keeps the no-push case a no-op.

One adjacent note, not a regression: `preserveLocalAsks`'s `local.size === 0`
fast path stops short-circuiting once the channel holds a *closed*
free-text-answered ask, because the widened raw scan (D2) sees its recorded
`customText` as a local pick worth collecting. That is exact parity with today's
closed *option*-answered asks, whose recorded `chosenOptionIds` already defeat
the same fast path; the per-ask `serverHasNoAnswer` guard inside
`preserveLocalAsks` still drops them from the actual replacement.

### D1 — the domain type reaches the full wire shape (DL-TBD-1)

`adaptAskQuestion` in `apps/ui/src/live/adapt.ts` today drops four wire fields
under the doc comment above it — "The wire's presentation/audit extras (header,
recommended, customText, timedOut) are not in the domain contract and are
deliberately dropped here rather than carried half-rendered" — and its option
map drops `AskOption.preview` the same way. The comment's own criterion — a
field is carryable exactly when the design renders it — was right, and this
record now satisfies it four times over: D4 binds `customText`, and D6 renders
`header`, `recommended`, and `preview`. Only `timedOut` still fails the
criterion (RIG-1310, Non-goals); it stays dropped, and the adapt comment is
rewritten to name only it.
...
— `AskQuestion` in `proto/compass/v1/comms.proto`. The domain already carries
the first answer-state field: `AskQuestion.chosenOptionIds` ("The chosen option
ids once answered; empty while pending (kept for audit)" in
`apps/ui/src/comms-stub.ts`). `customText` follows that precedent exactly, and
for the same two reasons:

1. **Local staging.** `answerAsk` records an option click by rewriting the local
   ask copy (`answerQuestion` in `apps/ui/src/store.ts` returns `{ ...q,
   chosenOptionIds: chosen }`), and every downstream mechanism — the stream-push
   carry (`preserveLocalAsks`), the refusal restage (D0), the send payload
   (`sendAsk`) — operates on that ask object. A parallel store map keyed by
   `(askId, questionId)` would need each mechanism re-implemented against a
   second state shape, and any one missed diverges silently (a push-carry that
   preserves clicks but discards text). On the object, the text inherits them.
...
The "half-rendered" hazard the adapt comment warns about no longer applies to
any of the four: each is fully rendered or bound by this design, which is the
comment's own criterion for carrying a field. The server-owned asymmetry on
`customText` is the same one `chosenOptionIds` already lives with: on a
*pending* ask the wire value is always empty (the proto's `AskQuestion`
answer-state banner: "Values supplied on an inbound Ask (via PostMessage) are
IGNORED" means the server never publishes a non-empty one before answering), so
local typing edits a field the stream will not fight over — and
`preserveLocalAsks` guards the push-race window exactly as it does for clicks.
...
`adaptAskQuestion` maps `header` and `customText` verbatim and `recommended:
w.recommended` (the generated field is already `number | undefined` —
`AskQuestion.recommended` in the generated `comms_pb.ts`); the option map gains
`preview: o.preview || undefined`, the same empty-string→absent convention
`description` uses on the line above in `adaptAskQuestion`.

A free-text-**only** question still needs **no new discriminant field**: it is
precisely `q.options.length === 0`, which the proto blesses ("Selectable
options; MAY be empty (a free-text-only question …)", `AskQuestion.options` in
`proto/compass/v1/comms.proto`) and the store's own comment already names ("a
custom_text-only answer to a free-text question, which carries no options to
choose", the doc comment above `serverHasNoAnswer` in `apps/ui/src/store.ts`).
Free text itself is on every question (D4); the option-less shape is only the
case where it is the sole answer path.

### D2 — store surface: a sibling `answerAskText`, recording locally (DL-TBD-2)

Widening `answerAsk(messageId, askId, questionId, optionId)` on `AppStore` in
`apps/ui/src/store.ts` is rejected: an option click and a keystroke are
...
- **Editable until submit — on every question.** The single-select
  first-responder-wins early return ("a single-select question settles on its
  first answer", inside `answerQuestion` in `apps/ui/src/store.ts`) reaches text
  only through exclusivity: the reducer sibling `answerQuestionText(q, text)`
  returns the same reference (the file's established no-op protocol, the doc
  comment above `answerQuestion`) when the question is a settled single-select
...
- **The click side of exclusivity.** `answerQuestion` (the option reducer in
  `apps/ui/src/store.ts`) widens: recording a pick on a single-select also
  clears the draft — `{ ...q, chosenOptionIds: chosen, customText: "" }` when
  `!q.allowMultiple` — so the staged state never holds the option-plus-text pair
  the server rejects (D4). On `allowMultiple` the text survives the toggle.
- **Gates.** Same as clicks: the `isAskSubmitted(askId)` guard at entry to
  `answerAsk`, the `ask.answered` guard inside the block map, and the
  `clearAskError(askId)` call when a record lands — all in
  `apps/ui/src/store.ts`.
- **`locked`/`closed`.** In `AskBlock`, `locked(q) = closed() ||
  (!q.allowMultiple && q.chosenOptionIds.length > 0)` (`locked` in
  `apps/ui/src/components/ChannelView.tsx`). The input's disabled state is
  exactly `locked(q())` — the same accessor the option buttons use, no new lock
  concept — and on a single-select this is the exclusivity surface: a chosen
  option disables the input, so type-after-pick is structurally impossible (D4).
  On `allowMultiple`, and on any question without a pick, text stays editable
  until submit; a closed ask locks the input and shows the recorded `customText`
  (D1's audit payoff).
...
- **`preserveLocalAsks`** — the unshipped-edit scan `if
  ...
  becomes `q.chosenOptionIds.length === 0 && q.customText === ""` — a typed
  draft is an unshipped edit a stream push must not discard. Raw compare, not
  trimmed: any typed character is worth carrying. (The `local.size === 0`
  fast-path consequence is in D0.)
...
q.customText.trim() !== ""`. Whitespace-only text does not count as answered:
the wire reads empty `custom_text` + no chosen ids as an accepted skip (the doc
comment above `serverHasNoAnswer` in `apps/ui/src/store.ts`), and a whitespace
"answer" is a skip to every reader; counting it answered would enable the submit
control over a question the wire will record as skipped.
...
(in `sendAsk` in `apps/ui/src/store.ts`). It becomes:
...
- **Coverage contract preserved** — still exactly one entry per question, in ask
  order ("Exactly one answer per AskQuestion in the Ask, keyed by question_id",
  `RespondToAskRequest` in `proto/compass/v1/comms.proto`); nothing about the
  map's shape changes.
- **Accepted-skip shape preserved** — a skipped question still ships
  ...
  coverage-of-every-question contract" (the doc comment above
  `serverHasNoAnswer` in `apps/ui/src/store.ts`). A skipped *free-text* question
  (never typed, or whitespace only) ships the same shape: `""` after trim.
...
The `respondToAsk` recorder inside `createFakeComms` in
`apps/ui/src/live/comms-fake.ts` widens its request type and its `askResponses`
copy to carry `customText`, so renderer/store tests can assert the wire payload
— today it records only `questionId` + `chosenOptionIds`. Any test asserting a
...
unconditional `Other (type your own)` and the proto's "free-text answering is
always available" (`AskQuestion.options` in `proto/compass/v1/comms.proto`) —
not only on the option-less ones. Matt's directive: full parity. The prior
draft's `options.length === 0` gate was this record's one deliberate scope cut,
and it is gone.
...
In `AskBlock` (in `apps/ui/src/components/ChannelView.tsx`), the per-question
body becomes the existing `.ask-options` `<For>` (rendered only when options
exist) followed unconditionally by the input:
...
- **Single-line, not multi-line.** The proto frames the field as the
  "Other"-chip answer ("Free-text answer (\"Other\")",
  `AskQuestionAnswer.custom_text` in `proto/compass/v1/comms.proto`; "mirroring
  the native tool's always-offered 'Other (type your own)'",
  `AskQuestion.options` in the same file) — a short typed alternative to picking
  an option, not a prose surface. The ask block is an inline channel element;
  long-form prose already has a home one gesture away (a channel reply). A
  single-line input accepts arbitrary length, just no line breaks. Multi-line
  growth is a named Open Question, non-load-bearing.
- **Binding and commit.** Controlled input: `value` reads the store's staged
  draft, `onInput` records per keystroke through `answerAskText`. One source of
  truth — the same local ask copy `preserveLocalAsks` protects — so a stream
  push landing mid-typing cannot eat the draft (the DOM value and the store
  never disagree). The per-keystroke `setComms` map over messages is the price;
  it is the same O(messages) write a click already performs (the click reducer
  in `answerAsk` in `apps/ui/src/store.ts`) at human typing rate.
...
- **Disabled/locked.** `disabled={locked(q())}` — identical accessor to the
  option buttons in `AskBlock`. After submit or on a closed ask the input is
  disabled and displays the recorded `customText` from the wire.
- **Accessibility.** The option buttons in `AskBlock` carry `aria-pressed`; a
  text input instead needs a label association. The question text lives in a
  bare `div.ask-question` with no id, so `aria-label={q().question}` provides
  the accessible name without minting id plumbing. `aria-pressed` is not
  applicable to a text input.
- **Hint line, three states.** The `choose any`/`choose one` hint in `AskBlock`
  must say that text is now an answer path everywhere: option-less — "type your
  answer · async — answer when ready"; single-select with options — "choose one,
  or type your own · async — answer when ready"; `allowMultiple` — "choose any,
  or type · async — answer when ready".
- **CSS.** `.ask-text` joins the ask family in `apps/ui/src/app.css` beside
  `.ask-option`, using the existing tokens (`--cx-text-dim`, `--cx-border-focus`
  family) — sizing to match the option row so a mixed ask reads as one block.

### D5 — the submit control: unconditional, with honest copy

This supersedes the prior draft's `canSubmit`. The control is no longer a skip
affordance that appears only in the partially-answered window (`canSubmit` in
`apps/ui/src/components/ChannelView.tsx`, `canSubmit = !closed() &&
answeredCount() > 0 && answeredCount() < ask().questions.length`) — it is the
**only** way any ask ever sends, so it renders on every live ask from first
paint (a send path the user must discover mid-flow is the empty-options bug
reborn one layer up).
...
- **Keyboard-operable — this is the primary path, not an accessibility
  ...
  activates on Enter and Space and fires `onClick` with no key handling of our
  own — so the work is not to build it but to **not break it**. Two things that
  would silently defeat it are absent and must stay absent: there is no `<form>`
  anywhere in `apps/ui/src` (so no implicit submission and no stray
  `type="submit"` default — keep `type="button"`), and the only `Enter`
  interception in the file is the `onKeyDown` inside `Composer` in
  `apps/ui/src/components/ChannelView.tsx`, a different component that never
  sees ask focus. Options are `<button>`s too, so Tab already reaches submit in
  DOM order with no `tabindex` needed. **A disabled button is not focusable**
  (measured), so on an untouched ask the control announces itself visually but
  cannot be tabbed to — acceptable, since an untouched ask has nothing to send.
...
- **`header` — a chip above the question.** Native calls it a "short display
  chip" (`omp://tools/ask.md`, `Question.header`). Rendered as `<div
  class="ask-header">` immediately above `.ask-question`, inside `<Show
  when={q().header}>` so an empty header (the wire's absent) renders no node.
  `.ask-header` joins the family in `apps/ui/src/app.css` above `.ask-question`:
  small, dim, chip-shaped — visually subordinate to the question text it labels.
...
- **`preview` — rendered, minimally, capped.** "Rich preview content (e.g.
  ...
  not answer state.
- **CSS.** `.ask-header`, `.ask-option-rec`, and `.ask-option-preview` join the
  `.ask-*` family in `apps/ui/src/app.css` beside `.ask-question`,
  `.ask-option`, and `.ask-option-desc`, on the existing tokens
  (`--cx-text-dim`, `--cx-text-faint`, `--cx-accent`).
...
2. **L2 (mandatory — the shipped-behaviour change).** A single-question ask's
   only click sends **nothing**; `submitAsk` then ships it. RED today: the click
   auto-sends (`answerAsk`'s send tail in `apps/ui/src/store.ts`), so
   `askResponses` is non-empty before any submit. Belongs here because the suite
   already owns the single-question send contract it inverts (the `a
   single-question ask still sends on its only click` case).
...
does to an in-progress ask", the file header comment of
`apps/ui/src/store.ask-race.test.ts`), using its `askMessage` builder:
...
2. **S2 — a push carrying a closed free-text ask wins over the draft.** Same,
   ...
   custom_text-only-closure case (the `a custom-text-only answered ask beats an
   unsubmitted local one` case) from "not restored" to "not restored *and* the
   draft yields". RED for the draft half.
3. **S3 (mandatory — the D0 restage contract). Split across two slices, because
   the mechanism lands before the field does.** Hold the respond in flight
   (`fake.holdNextAskResponse`), `submitAsk`, then emit a blank unanswered
   restatement of the ask (adopted, because the submitted ask is skipped by the
   preserve), then reject the held respond. Assert the staged answer is back and
   the ask is retryable (`isAskSubmitted` false). RED against the pre-change
   tree and against a change that omits the restage: the state after refusal is
   the adopted blank ask. Belongs in this suite: it is precisely an
   `adoptComms`-vs-local interaction, beside the other held-in-flight case (the
   `a refusal after the server accepted does not reopen the closed ask` case).
...
**`apps/ui/src/components/ChannelView.ask.test.tsx`** — the render surface over
the live fake, whose `freeText` option already builds option-less wire questions
(the doc comment above `wireAskMessage` and `wireAskMessage` itself in
`apps/ui/src/live/comms-fake.ts`), same `mountAsk`/`settled` harness
(`FakeComms` in the same file):
...
5. **R5 — the submit control is unconditional on a live ask.** From first
   render: control present and disabled; after one answer: enabled, "submit —
   skip the rest"; after all answers: enabled, "submit"; after submit: gone.
   RED: today the control renders only in the partially-answered window
   (`canSubmit` in `apps/ui/src/components/ChannelView.tsx`).
...
- `answerAsk sends nothing until the ask is complete, then one full
  RespondToAsk` — leg (b) asserts the completing click ships the respond.
  **Inverts:** the completing click now also asserts `askResponses` empty; an
  appended `submitAsk` carries the send assertion. Rename accordingly
  (`answerAsk never sends; submitAsk ships the full respond`).
- `a single-question ask still sends on its only click` — **inverts wholesale**
  (this is L2's subject): the click sends nothing; submit sends. Its comment
  ("the gate must not regress the ONE-question ask") pins exactly the behaviour
  Matt removed.
- `submitAsk sends an incomplete ask with the skipped question empty` —
  **survives unchanged**; its trigger is already the explicit path.
- `a completed ask takes no further answer and issues no second respond` — the
  send trigger is two completing clicks. **Edit:** insert a `submitAsk` to issue
  the one respond; the further-answer/further-submit inertness legs survive.
- `a refused RespondToAsk rolls the local answer back and stays retryable` —
  **inverts in meaning:** the send becomes click + `submitAsk`, and the "local
  record does NOT show the refused answer" leg inverts — submit records nothing,
  so a refusal leaves the *staged answer in place* (still shown, still
  retryable). The re-answer/re-send tail becomes a plain re-submit.
- `a refused respond does not clobber an ask the stream moved meanwhile` — send
  gesture becomes submit; the contract **survives** (the D0 restage declines:
  the mid-flight push carries an accepted — answered — value, and
  `!current.answered` blocks the restage exactly as it blocked the rollback).
- `a rejected single-select answer does not reach the wire` — send trigger (the
  completing q-2 click) becomes click + submit; the first-responder-wins wire
  assertion survives.
- `an unknown ask coordinate sends no RespondToAsk` — survives textually, but
  its send-side assertion becomes vacuous (no click ever sends now); keep it as
  records-nothing coverage, with the comment updated.

**`apps/ui/src/store.ask-race.test.ts`** (the suite carrying 8 `askResponses`
assertions):

- `an unsubmitted local answer survives a stream push` — the "still completable"
  leg sends via the completing click. **Edit:** click then `submitAsk`; the
  survives-the-push assertions stand.
- `an authoritative server answer beats an unsubmitted local one` — no send
  trigger; **survives**.
- `a shipped ask takes the pushed server value` — ships via a single-question
  ask's only click. **Edit:** click + submit.
- `a fully-skipped answered ask beats an unsubmitted local one` — **survives**
  (asserts `askResponses` empty after a first click, which the new model
  satisfies a fortiori).
- `a custom-text-only answered ask beats an unsubmitted local one` —
  **survives**; S2 extends it.
- `an unanswered pushed ask still preserves the local answer` — the "real state,
  not a ghost" leg sends via the completing click. **Edit:** click + submit.
- `a click on a server-closed ask ships nothing` — **survives**; its comment
  (the click "would ship the doomed respond") updates — the guarded contract is
  now the *record* refusal (`chosenIn` stays empty), the wire leg being vacuous.
- `a submit on a server-closed ask ships nothing` — **survives unchanged**, and
  gains weight: submit is now the only send path this gate defends.
- `a pushed ask that grew a question beats the local shape`, `a pushed ask that
  renamed a question beats the local shape`, `a push over an untouched ask is
  adopted by reference`, `a pushed ask that revised its options beats the local
  pick` — no send trigger; **survive**.
- `a refusal after the server accepted does not reopen the closed ask` — the
  held-in-flight send is a single-question click. **Edit:** click + submit; the
  declined-restage behaviour is identical to the declined rollback it pins
  (guarded by `answered` either way).

**`apps/ui/src/components/ChannelView.ask.test.tsx`:**

- `the first click sends nothing; the completing click sends one full respond` —
  second leg **inverts** (R4's subject): the completing click asserts empty; the
  submit control's click carries the send assertion.
- `the submit control appears only on a partially answered ask` — **inverts
  wholesale** (R5's subject): the control now renders on every live ask; the
  visibility legs become the disabled/enabled/copy legs.
- `submitting a partially answered ask ships the skipped question empty` —
  **survives**; already submit-triggered.
- `a refused respond renders in the ask block and clears on the next answer` —
  the send is two completing clicks. **Edit:** clicks + submit-control click;
  the rollback-shaped leg ("q-2 can be answered again") relaxes — with no
  rollback, the staged q-2 answer survives the refusal, and the
  clears-on-next-answer leg re-answers or re-submits.
- `a server-closed ask renders locked with no submit control` — **survives**:
  the no-control assertion now rides the `!closed()` visibility gate rather than
  the partial-answer window, same observable.

**`apps/ui/src/store.test.ts`** — the `answerAsk` describe runs against the
*offline* store (`withStore`, no comms client) and asserts only local recording
(first-responder-wins, unknown-coordinate no-ops). **Survives unchanged** —
nothing in it observes a send.

**`apps/ui/src/comms.test.ts`** — references `answerAsk` only in the doc comment
above `COMMS_ASKS` ("the answerAsk MUTATION path lives in store.test.ts and is
not restated here"). **Unaffected.**

**`apps/ui/src/components/RightSidebar.fleetpane.test.tsx`** — references
`answerAsk` only in the doc comment above `mountRightSidebar`; the ask test
...
(`.ask-option` queries, `store.answerAsk`, the `locked`/`chosen` render) against
the *offline* store (`createAppStore({ initialComms: STUB_COMMS_STATE })` in
`mountTopicView` and in `mountChannelView`), so it observes local recording only
and never a send. **Survives D0 unchanged** — it cannot pin auto-send. T3 adds
DOM beneath these mounts (the input on every question, the new hint copy, the
unconditional submit control) without changing the `.ask-option` queries or
first-responder-wins, so no edit is expected; listed because silence here would
leave an implementer unsure whether a break in it is intended.

## Plan

### Global Constraints

- TypeScript strict: no `any`, no `as`, no `@ts-ignore` (the one sanctioned `as
  unknown as CommsClient` seam in `createFakeComms`
  (`apps/ui/src/live/comms-fake.ts`) stays as-is).
...
Tests: L2 and R6 (new), S3a (the clicks-only restage guard — the mechanism ships
here, so its field-agnostic half is testable here), plus every inversion/edit in
D7's enumeration for `store.live.test.ts`, `store.ask-race.test.ts`, and
`ChannelView.ask.test.tsx` (the `the first click sends nothing`, `the submit
control appears only on a partially answered ask`, `a refused respond renders in
the ask block`, and completed-ask edits).
