...
- writes the value into the provider via `secrets.Resolver.Set`
  (`go/internal/secrets/resolver.go:236`, `func (r *SpecResolver) Set(ctx
...
required key" (resolver.go:109-111; the function, resolver.go:115-138);
...
resolver.go:88, but C1 does not use it), and BY DEFAULT one provider URI
...
- **Mechanism C2: separate table + a separate SecretSpec PROFILE —
  ...
  resolver pinned to its own profile (`WithProfile`, resolver.go:88) so even
...
- The master key NEVER appears in the DB, in logs, in argv (Set feeds stdin,
  resolver.go:212-213), or in error strings.
...
- `Interfaces:`
  ...
  - Value custody + operator provisioning (Matt-ruled, F2): the VALUES of all
    ...
    0.17` for the READ path (`b.Load()`, resolver.go:171); AND (b) the
    `secretspec` CLI BINARY the WRITE path shells via `resolver.Set`
    (`exec.CommandContext(ctx, r.cli, args...)`, resolver.go:272; `r.cli`
    defaults to bare `"secretspec"` on PATH, resolver.go:29/101) — also `>=
    0.17` built with the `age` feature and pinned explicitly via `WithCLI`
    (resolver.go:91) into the Server's closure so the read and write halves
    ...
    `SERVER_LINEAR_FORGE_CLIENT_SECRET`) and the provider keyspace is keyed by
    NAME (`setArgs`, resolver.go:321-327), so a value is populated under the
...
- `Interfaces:`
  ...
  - Key encoding for provider storage: base64(std) of the 32 raw bytes
    (SecretSpec values are strings; `Set` rejects empty, resolver.go:241-243).
...
- `Interfaces:`
  - `func provisionGatewayMasterKey(ctx context.Context, resolver secrets.Resolver, st *store.Store) (envelope.Key, error)`
    ...
    (resolver.go:236; the value rides stdin, never argv,
    resolver.go:212-213) → `st.DeclareServerSecret(ctx, "", name)` with
  ...
  - **Bounded critical section (mandatory — a stuck provider must not wedge
    ...
    `exec.CommandContext(ctx, r.cli, …)` (resolver.go:272), so a ctx deadline
    genuinely kills it. `SpecResolver.Resolve` is NOT: it threads ctx only into
    `DeclaredSecrets` (resolver.go:146); the actual provider round-trip is
    `b.Load()` (resolver.go:171), whose SDK signature carries NO ctx
  ...
  - **Read-back verify, every boot:** after provisioning AND on every
    ...
    uncancellable `Load` (resolver.go:171) and uses the SAME bounded-offload
  ...
  - **Master-key overwrite guard (F1 prefix partition — on EVERY
    ...
    (resolver.go:321-327) against the keyspace that is SHARED under the default
...
- [ ] T0 — `server_secrets` store (mechanism C1): new migration (table +
      ...
      the Go module for the SDK READ path (`b.Load()`, resolver.go:171) AND
      the `secretspec` CLI BINARY the WRITE path runs (`resolver.Set` shells
      `r.cli`, resolver.go:272/29), the latter pinned via `WithCLI`
      (resolver.go:91) into the Server's closure so read and write cannot drift
