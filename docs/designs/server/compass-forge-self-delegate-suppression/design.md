...
> DL-364 | Forge notifications are self-origin-suppressed at the notify-router
...
Plus the owner-carriage amendment (**DL-365**), which T0 implements. DL-094 is
...
> DL-365 | `compass.v1.AgentAttribution` regains `owner_handle` (field 2, the
...
> matching consumer; DL-364's suppression is the first. Attribution stays a
...
immutable): DL-365 carries the amended-clause scope as a new row, and DL-186
...
threads `ownerHandle: w.ownerHandle`, re-points the comment at DL-365, and
updates both assertions. (`verified` stays hedged — DL-094's
attribution-is-not-a-trust-claim core is unchanged, and DL-365 amends only the
owner clause.)
...
`docs/designs/DECISIONS.md` in this record's freeze PR — **DL-364** (the
suppression decision) and **DL-365** (the `owner_handle` carriage amendment T0
