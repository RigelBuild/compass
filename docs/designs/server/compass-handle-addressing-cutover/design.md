...
this door); `IssueToken` (`service.go:416`) names a user OR agent → a bare
...
`IssueToken` (`service.go:416`) resolves `account_handle` without the D9 clip
(ruling 9): a bare handle via `UserByHandle`, `owner/agent` via `UserByHandle`
then `AgentByHandle`, the same lookups Spawn and Provision use (it may name a
user or an agent; amended 2026-09-25, originally the `AccountsByHandles` path).
Because this is the
...
9. **IssueToken visibility (RIG-4019) — RULED no clip on the admin door
   ...
   so the clip would make a token for an unrelated user's agent NOT_FOUND, a
   power the id-keyed door always had. D9 governs what a comms caller can see;
