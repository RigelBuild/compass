...
- **[Non-load-bearing] Record placement.** This record establishes
  `docs/designs/platform/` in a repo whose architecture docs live in
  `docs/architecture/` (org convention: `docs/designs/<domain>/`). Deferred
  with rationale: T4 folds the durable content into `build-and-ci.md` once
  implemented; the design record stays where the org convention puts it.
