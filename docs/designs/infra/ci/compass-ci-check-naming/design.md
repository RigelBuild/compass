...
`## Infrastructure & CI` section heading. **The ids below are placeholders, not
reservations.** Ledger ids are not reserved — final assignment is merge-order
dependent (Matt drives the gate), and several design PRs append concurrently, so
this record sits behind OQ1's ruling and merges after the in-flight ledger PRs
(#664/#645/#653/#662). The applying procedure at rebase-time: after this record's
PR rebases onto merged main, take the next contiguous free pair confirmed against
that merged `DECISIONS.md` (not against any earlier mesh estimate), fill it into
both rows below and this note, and apply to `docs/designs/DECISIONS.md` in the
same PR. Merged main tops out at DL-268 (PR #664's RIG-2794 healed the DL-264
duplicate), but DL-269..276 is the known in-flight block (PR #645 takes 269-271,
merge-ready; PR #662 takes 272-275; PR #653 takes 276). Since this record is
OQ1-gated it merges last, so it sits above that block at **DL-277/DL-278** (the
ledger gate tolerates gaps, rejecting only duplicates) — recomputed at the final
pre-merge rebase:
