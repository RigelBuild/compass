...
// KernelEnvVar, when set, points the harness at the guest kernel image to boot,
// defaulting otherwise to the `nix build .#compass-guest-kernel` result symlink.
const KernelEnvVar = "COMPASS_TEST_GUEST_KERNEL"

// RootfsEnvVar, when set, points the harness at the guest rootfs image to boot,
// defaulting otherwise to the `nix build .#compass-guest-rootfs` result symlink.
...
// the VMM/virtiofsd binaries. A binary missing from PATH is a hard failure on a
// run that has already cleared the KVM gate: the guest images and VMM are
// prerequisites of any boot, so their absence is a misconfigured environment,
// not a reason to silently skip into a green suite.
