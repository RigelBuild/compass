...
collect:
	for len(seen) < n {
		...
		case ref := <-got:
			if !valid[ref.RowID] {
				t.Errorf("delivered %q, which was never published", ref.RowID)
			}
			seen[ref.RowID]++
		case <-drain.Done():
			break collect
	...
	if len(seen) < 2 {
		t.Errorf("drain delivered %d event(s), want at least 2 — Unsubscribe discarded the buffered events instead of running them", len(seen))
	}
	for id, count := range seen {
		if count != 1 {
			t.Errorf("%s was delivered %d time(s), want exactly 1 — Unsubscribe duplicated a claimed event", id, count)
		}
	}
}
