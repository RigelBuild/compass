...
// RelayBoardCall forwards one agent-initiated board call (write an issue's
// canonical lifecycle state) into the hub, which resolves the relayed
// session_id to its bound agent account (the caller) and delegates under that
// account, fail-closed (relay_board.go). The bearer interceptor has already
...
func (h *Handler) FetchSecrets(ctx context.Context, req *connect.Request[compassv1internal.FetchSecretsRequest]) (*connect.Response[compassv1internal.FetchSecretsResponse], error) {
	...
	// container_name authorizes the PROVISION-time initial materialize; a
	// session_id authorizes the post-Start rotation re-fetch. A foreign/unknown
	// selector — or none — is rejected CodePermissionDenied.
