...
func TestT0KeyspacePartition(t *testing.T) {
	...
	for _, name := range []string{"SERVER_LINEAR_FORGE_CLIENT_SECRET", "GATEWAY_CREDENTIALS_MASTER_KEY"} {
		err := s.DeclareSecret(ctx, actor, name, SecretScopeTenant, "", SecretDeliveryEnv, SecretKindGeneric, "", "")
	...
	if err := s.DeclareSecret(ctx, actor, "PLAIN_USER_TOKEN", SecretScopeTenant, "", SecretDeliveryEnv, SecretKindGeneric, "", ""); err != nil {
