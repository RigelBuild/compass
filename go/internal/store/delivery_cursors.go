...
// subscribe upsert) and by the exported SeedDeliveryCursor wrapper. The seed is
// self-guarding and idempotent in one race-free statement (queries/
// delivery_cursors.sql SeedDeliveryCursor): the WHERE EXISTS admits the row only
// for an agent account (a user id yields zero rows, so a non-agent member is a
// silent no-op rather than an FK violation), it seeds acked_seq to the current
// channel head — MAX(seq) over the channel's messages, 0 if empty — with NO
// history replay (design record D2), and ON CONFLICT DO NOTHING means a
// re-subscribe never resets an existing cursor. Safe to call unconditionally for
// a member whose agent-ness is not separately known.
func seedDeliveryCursor(ctx context.Context, tx pgx.Tx, agent AccountID, channel ChannelID) error {
	if err := db.New(tx).SeedDeliveryCursor(ctx, db.SeedDeliveryCursorParams{
...
// it seeds EVERY agent member of the channel in a single statement (queries/
// delivery_cursors.sql SeedChannelDeliveryCursors — the set form of the per-row
// seed, collapsing the per-member seed loop), riding the caller's existing
// transaction so a missed seed is a loud failure in that same commit. The JOIN
// agent_accounts is the agent-only guard (a human member has no agent_accounts
// row and so is a silent no-op rather than an FK violation), each row is seeded
// to the current channel head with NO history replay (design record D2), and
// ON CONFLICT DO NOTHING keeps a re-subscribe / re-run idempotent — it never
// resets an existing cursor. Safe to call for a channel whose member set includes
// humans.
func seedChannelDeliveryCursors(ctx context.Context, tx pgx.Tx, channel ChannelID) error {
	if err := db.New(tx).SeedChannelDeliveryCursors(ctx, string(channel)); err != nil {
...
func (s *Store) RecordOwedMention(ctx context.Context, agent AccountID, channel ChannelID, messageID string) error {
	if err := s.q.RecordOwedMention(ctx, db.RecordOwedMentionParams{
		AgentAccountID:   string(agent),
		MessageID:        messageID,
		ChannelID:        string(channel),
...
func (s *Store) OwedMentions(ctx context.Context, agent AccountID) (map[ChannelID][]Message, error) {
	rows, err := s.q.OwedMentions(ctx, string(agent))
	if err != nil {
	...
	for _, r := range rows {
		...
		out[ChannelID(r.ChannelID)] = append(out[ChannelID(r.ChannelID)], m)
	}
	return out, nil
}
...
func (s *Store) clearOwedMention(ctx context.Context, tx pgx.Tx, agent AccountID, messageID string) (cleared bool, err error) {
	affected, err := db.New(tx).ClearOwedMention(ctx, db.ClearOwedMentionParams{
		AgentAccountID: string(agent),
		MessageID:      messageID,
	})
	if err != nil {
...
func (s *Store) ClearOwedMention(ctx context.Context, agent AccountID, messageID string) error {
	if _, err := s.q.ClearOwedMention(ctx, db.ClearOwedMentionParams{
		AgentAccountID: string(agent),
		MessageID:      messageID,
	}); err != nil {
...
func (s *Store) CountOwedMentions(ctx context.Context) (int, error) {
	n, err := s.q.CountOwedMentions(ctx)
	if err != nil {
...
func (s *Store) MarkMentionsRouted(ctx context.Context, messageID string) error {
	if err := s.q.MarkMentionsRouted(ctx, db.MarkMentionsRoutedParams{
		MentionsRoutedAt: pgtype.Int8{Int64: time.Now().UnixMilli(), Valid: true},
		ID:               messageID,
	}); err != nil {
...
func (s *Store) UnroutedMentionMessages(ctx context.Context, afterSeq int64, limit int) ([]MessageWithChannel, error) {
	rows, err := s.q.UnroutedMentionMessages(ctx, db.UnroutedMentionMessagesParams{
		Seq:   afterSeq,
		Limit: int32(limit), //nolint:gosec // G115: caller passes a small fixed batch size
	})
	if err != nil {
...
func (s *Store) AckDelivery(ctx context.Context, agent AccountID, channel ChannelID, messageID string) error {
	...
	qtx := db.New(tx)
	seq, err := qtx.ResolveAckMessage(ctx, db.ResolveAckMessageParams{ID: messageID, ChannelID: string(channel)})
	switch {
	case noRows(err):
	...
	cursor, err := qtx.LoadDeliveryCursor(ctx, db.LoadDeliveryCursorParams{
		AgentAccountID: string(agent),
		ChannelID:      string(channel),
	})
	switch {
	...
	if err := qtx.AdvanceDeliveryCursor(ctx, db.AdvanceDeliveryCursorParams{
		AgentAccountID: string(agent),
		ChannelID:      string(channel),
		AckedSeq:       ackedSeq,
		AboveSeqs:      remaining,
...
func (s *Store) InSweepSet(ctx context.Context, agent AccountID, channel ChannelID) (bool, error) {
	in, err := s.q.InSweepSet(ctx, db.InSweepSetParams{
