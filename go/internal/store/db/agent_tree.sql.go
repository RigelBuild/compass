...
const agentNeighborhood = `-- name: AgentNeighborhood :many
...
type AgentNeighborhoodRow struct {
	ID              string
	Handle          string
	DisplayName     string
...
func (q *Queries) AgentNeighborhood(ctx context.Context, id string) ([]AgentNeighborhoodRow, error) {
	rows, err := q.db.Query(ctx, agentNeighborhood, id)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []AgentNeighborhoodRow
	for rows.Next() {
		var i AgentNeighborhoodRow
		if err := rows.Scan(
			&i.ID,
			&i.Handle,
			&i.DisplayName,
...
const agentSubtree = `-- name: AgentSubtree :many
...
func (q *Queries) AgentSubtree(ctx context.Context, accountID string) ([]AgentSubtreeRow, error) {
	rows, err := q.db.Query(ctx, agentSubtree, accountID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []AgentSubtreeRow
	for rows.Next() {
		var i AgentSubtreeRow
		if err := rows.Scan(
			&i.ID,
			&i.Handle,
			&i.DisplayName,
...
const agentsByOwner = `-- name: AgentsByOwner :many
SELECT a.id, a.handle, a.display_name,
       u.role AS user_role,
       ag.owner_user_id, ag.home_channel_id, ag.persona, ag.role AS agent_role, ag.parent_agent_id,
       sy.account_id AS system_account_id
...
func (q *Queries) AgentsByOwner(ctx context.Context, ownerUserID string) ([]AgentsByOwnerRow, error) {
	rows, err := q.db.Query(ctx, agentsByOwner, ownerUserID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []AgentsByOwnerRow
	for rows.Next() {
		var i AgentsByOwnerRow
		if err := rows.Scan(
			&i.ID,
			&i.Handle,
			&i.DisplayName,
