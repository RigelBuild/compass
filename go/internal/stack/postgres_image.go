...
// opens a reviewable PR to advance the digest. A MAJOR bump (18 -> 19) amends
// DL-260's `postgres:18` choice (on-disk-format stability), so it stays manual: the
// customManager's packageRule pins allowedVersions to /^18$/, so 18 -> 19 is never auto-proposed.
