...
// Quit-lifecycle gate. The explicit "Quit and stop stack" orchestration is
...
// quit-anyway-on-failure default (OQ-6) are all verified deterministically, with
// no real compass-stack exec and no display.
...
// TestStopStackAndQuitQuitsAnywayOnDownFailure: the OQ-6 default. When
// compass-stack down FAILS, the app STILL quits exactly once (a lingering stack
// is the safe failure; trapping the user in a live window is worse).
